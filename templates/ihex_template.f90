...
C
C     FOR PREC_TYPE, OPEN CORE POINTERS MUST BE MODIFIED
C
      IZ = IZS/2 + 1
      NZ = NZS/2 + 1
C
C     THIS ROUTINE OPERATES IN PREC_TYPE.
...
C
      ...
      CALL TRANS_FUNC (EST(M),TK)
      DO 101 L = 1,3
  101 Z(IZ+L-1) = CAST_FUNC(RZ(J+L-1)*0.25)
      CALL GMMAT_FUNC (TK,3,3,0,Z(IZ),3,1,0,Z(N))
      GPTLD(I) = 0.25*GPTLD(I)
      GO TO 104
  102 DO 103 L = 1,3
  103 Z(N+L-1) = CAST_FUNC(RZ(J+L-1)*0.25)
      GPTLD(I) = 0.25*GPTLD(I)
  104 CONTINUE
C
C     REARRANGE BGPDT
...
C
      CALL IHEXS_FUNC (TYPE,Z(IN),Z(IG),JACOB,DETJ,EID,S(I),S(J),S(K),
     1             EST(BCORD))
...
C
      CALL GMMAT_FUNC (Z(IG),NGP,3,0,JACOB,3,3,0,Z(IX))
C
...
C
      ...
      TEMP = 0.0
      DO 1110 L = 1,NGP
 1110 TEMP = TEMP + Z(IN+L-1)*CAST_FUNC(GPTLD(L))
      TEMP = TEMP - CAST_FUNC(TREF)
      IF (ANIS) GO TO 1115
      SIG(1) =-CAST_FUNC(TALPHA)*(E1+2.0*E2)*TEMP
      SIG(2) = SIG(1)
      SIG(3) = SIG(1)
      SIG(4) = 0.0
      SIG(5) = 0.0
...
C
C     STORE ALPHA IN PREC_TYPE
C
      DO 1116 IJK = 1,6
 1116 DALPHA(IJK) = BUFM6(IJK+37)
C
      CALL GMMAT_FUNC (GMAT,6,6,0, DALPHA,6,1,0,SIG)
      DO 1117 IJK = 1,6
...
C
 ...
 1146 STORE(IJK) = 0.D0
      ...
      STORE(18) = Z(II+1)
C
      CALL GMMAT_FUNC (GMAT,6,6,0,STORE(1),6,3,0,STR)
C
C     ============================================================
C
 1150 CALL GMMAT_FUNC (STR,6,3,-2,Z(ID+3*L-3),3,1,0,SIG)
 1160 CONTINUE
      STR(1) = SX
      SX = SX + SY
      SY = SY + SZ
...
C
 ...
 1223 STORE(IJK) = 0.D0
      ...
      STORE(18) = Z(MZ  )
C
      CALL GMMAT_FUNC (STORE(1),3,6,0,GMAT(1),6,6,0,STORE(19))
C
C     STORE CJ
C
      DO 1224 IJK = 1,18
 1224 STORE(IJK) = 0.D0
      ...
      STORE(18) = Z(NZ  )
C
      CALL GMMAT_FUNC (STORE(19),3,6,0,STORE(1),6,3,0,STORE(37))
      IJKL = 0
      DO 1225 IJK = 1,3
      DO 1225 IJL = 1,3
      IJKL = IJKL + 1
...
C
      Z(L) = Z(L) + CAST_FUNC(RHO)*SFACT*Z(IN+M-1)*Z(IN+N-1)
 1400 CONTINUE
 2000 CONTINUE
C
...
C
      DO 2025 I = 1,NGP
      J = IN + (I-1)*9
      CALL TRANS_FUNC (EST(BGPDT+4*I-4),Z(J))
 2025 CONTINUE
...
C
 ...
 2119 CONTINUE
      CALL GMMAT_FUNC (Z(MZ),3,3,1,Z(NZ),3,3,0,TF)
C
...
C
 ...
 3030 RZ(IZS+I) = EST(BCORD+I-1)
      CALL TRANS_FUNC (RZ(IZS),Z(IT))
      DO 3040 I = 1,3
 3040 RZ(IZS+I) = -RZ(IZS+I)
      CALL TRANS_FUNC (RZ(IZS),Z(IN))
      DO 3050 I = 1,9
...
C
      DO 3060 I = 1,6
 3060 Z(IZ+I+1) = CAST_FUNC(KHEAT(I))
      L  = IZ + 2
      M  = L + 3
      N  = M + 2
      NZ = IT
...
C
      CALL TRANS_FUNC (RZ(IZS),Z(IT))
      DO 3150 L = 1,6
 3150 Z(IZ+L+1) = CAST_FUNC(KHEAT(L))
      NZ = IT
      L  = IZ + 2
      M  = L + 3
      N  = M + 2
...
C
 ...
 3170 PRT1 = PRT1 + Z(IX+3*M+L-4)*Z(IX+3*N+L-4)
      PRT1 = SFACT*CAST_FUNC(KHEAT(1))*PRT1
      GO TO 3190
C
C     ANISOTROPIC CASE
C
...
C
      Z(L) = Z(L) + SFACT*CAST_FUNC(CP)*Z(IN+M-1)*Z(IN+N-1)
 3220 CONTINUE
