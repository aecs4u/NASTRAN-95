...
C
      ...
      REAL        CONST,BL22,BLSQ3,FM,KE,KK,KEP,M,MEP,ME,
...
C
C     DETERMINE IF WE HAVE NON-0.0 OFFSET VECTORS.
C
      AOFSET = .TRUE.
      J = JOFSTA - 1
      DO 70 I = 1,3
...
C
C     SINCE THERE WAS AT LEAST 1.0 NON-0.0 OFFSET VECTOR RECOMPUTE VECI
C
  140 DO 145 I = 1,3
      JTA = I - 1 + IDELA
      JTB = I - 1 + IDELB
...
C
C     IF ELASTICITY AND SHEAR MODULES BOTH 0.0, SKIP STIFFNESS
C     CALCULATION
C
...
C
      ...
      GAK1 = G*A*K1
      R1   = (12.0*EI1*GAK1)/(GAK1*BLCUBE + 12.0*BL*EI1)
      GO TO 220
  210 R1   = 12.0*EI1/BLCUBE
  220 IF (K2.EQ.0.0 .OR. I12.NE.0.0) GO TO 230
      GAK2 = G*A*K2
      R2   = (12.0*EI2*GAK2)/(GAK2*BLCUBE + 12.0*BL*EI2)
      GO TO 240
  230 R2  = 12.0*EI2/BLCUBE
C
  240 SK1 = 0.25*R1*BLSQ + EI1/BL
      SK2 = 0.25*R2*BLSQ + EI2/BL
      SK3 = 0.25*R1*BLSQ - EI1/BL
      SK4 = 0.25*R2*BLSQ - EI2/BL
C
      AEL = A*E/BL
      LR1 = BL*R1/2.0
      LR2 = BL*R2/2.0
...
C
C     DETERMINE IF THERE ARE NON-0.0 PIN FLAGS.
C
  305 KA = IECPT(JPINA)
      KB = IECPT(JPINB)
...
C
  ...
  340 CONTINUE
C
C     DIVIDE KE INTO 4.0 SUBMATRICES AND STORE IN OPEN CORE
C
C      E                   E                   E
...
C
C     0.0 OUT THE ARRAY WHERE THE 3X3 MATRIX H AND THE W  AND W  6X6
...
C
      ...
      IG = 19
C
C     IF THERE IS A NON-0.0 OFFSET FOR THE POINT, SET UP THE D 3X3
C     MATRIX.
C
...
C
  ...
  400 CONTINUE
C
C     REFORM THE K MATRIX (12X12) FROM THE 4.0 SUBMATRICES (6X6) AND
...
C
  600 CONST = (FL*(RHO*A +  NSM))/
     1        420.0
      IF (IMASS.EQ.0 .OR. CONST.EQ.0.0) RETURN
...
C
  ...
  670 ME(K) = MEP(K)
C
C     0.0 OUT THE IPIN(J) TH ROW AND COLUMN OF ME
C
  680 J1 = JJ - IPIN(J)
      J2 = IPIN(J)
      DO 690 K = 1,12
...
C
  ...
  810 MEP(I) = 0.0
      FM = 0.5*FL*(RHO*A + NSM)
C
C     DETERMINE IF THE GRID POINT IS ASSOCIATED WITH A NON-0.0 OFFSET.
C
      JOFSET = 9
      DO 850 II = 1,2
      IX = (II-1)*36
...
C
  ...
  830 MEP(IS) = MEP(IS)*FM
      GO TO 850
C
C     HERE WE HAVE A 0.0 OFFSET VECTOR
C
  840 MEP(IX+ 1) = FM
      MEP(IX+ 8) = FM
      MEP(IX+15) = FM
...
C
      CALL HMAT (IELID)
      KK(1) = (FK*ECPT(17))*FL/2.0
      IF (KK(1) .EQ. 0.0) RETURN
      KK(2) = KK(1)
      DICT(2) = 2
...
!
! 3. Matrix Operations:
!    TRANSS → TRANSD or TRANSS
!    GMMATS → GMMATD or GMMATS
!
! 4. Type Conversions (critical for mixed precision):
!    E → DBLE(E) (double) or E (single)
!    Used extensively: G*A*K1
!
! 5. Numeric Literals:
!    0.0 → 0.0D0 or 0.0
!    1.0 → 1.0D0 or 1.0
...
!    35.0 → 35.0D0 or 35.0
!    54.0 → 54.0D0 or 54.0
!    156.0 → 156.0D0 or 156.0
!    175.0 → 175.0D0 or 175.0
!    420.0 → 420.0D0 or 420.0
!    1.0E-18 → 1.0D-18 or 1.0E-18
!    1.0E-7 → 1.0D-7 or 1.0E-7
!
