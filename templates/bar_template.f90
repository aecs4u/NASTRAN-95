!===============================================================================
! TEMPLATE: bar_template.f90
!
! PURPOSE:
!   Single-source template for BAR (beam) element stiffness and mass matrices.
!   Generates both BARD (double precision) and BARS (single precision) from
!   this single source file.
!
! BUILD:
!   # Generate BARD (double precision):
!   gfortran -cpp -E -DDOUBLE_PRECISION bar_template.f90 -o bard.f
!
!   # Generate BARS (single precision):
!   gfortran -cpp -E bar_template.f90 -o bars.f
!
! TEMPLATE SYSTEM:
...
!     - CAST_FUNC(X) for type conversions (DBLE in double, identity in single)
!     - ZERO, ONE, TWO, HALF, etc. for numeric literals
!
! ELEMENT FEATURES:
!   - Beam element with 12 DOF (6 per node: 3 translations, 3 rotations)
!   - Axial, bending (2 directions), torsional, and shear stiffness
!   - Offset vectors at both ends
!   - Pin flags for releasing DOF
!   - Consistent and conventional mass matrices
!   - Heat transfer support (conductivity and capacity)
!
! ORIGINAL SOURCES:
!   - bard.f (908 lines) - Double precision
!   - bars.f (880 lines) - Single precision
!   - 97.2% duplication eliminated by this template
!
...
#include "precision_macros.h"
      ...
      SUBROUTINE BAR##PREC_SUFFIX
C
#ifdef DOUBLE_PRECISION
C     DOUBLE PRECISION VERSION
C     Generated from bar_template.f90
#else
...
C
      ...
      PREC_TYPE        CONST,BL22,BLSQ3,FM,KE,KK,KEP,M,MEP,ME,
     ...
     3                 (VEC(7),VECK(1))
      DATA    IKK   /  1,7,73,79   /, EPSI,EPSI2 / SMALL_TOL,LARGE_TOL /
...
C
      ...
      ICSIDA = IECPT(34)
      ICSIDB = IECPT(38)
      LIMIT  = CAST_FUNC(IABS(KSY87))*LITERAL_01
C
...
C
   ...
   40 FL = FL + SMALLV(I)**2
      FL = SQRT_FUNC(FL)
      IF (ABS_FUNC(FL) .LT. EPSI) GO TO 7770
      DO 50 I = 1,3
...
C
      IF (ICSIDA .NE. 0) CALL TRANS_FUNC(ECPT(JCSIDA),TA)
      IF (ICSIDB .NE. 0) CALL TRANS_FUNC(ECPT(JCSIDB),TB)
C
...
C     TO B
C
      A2B = SQRT_FUNC(VECI(1)**2 + VECI(2)**2 + VECI(3)**2)
C
...
C
  ...
  110 DELA(I) = ECPT(J)
      ...
      IDELA = 4
      CALL GMMAT_FUNC (TA, 3,3,0, DELA(1),3,1,0, DELA(4))
C
...
C
  ...
  130 DELB(I) = ECPT(J)
      ...
      IDELB = 4
      CALL GMMAT_FUNC (TB, 3,3,0, DELB(1),3,1,0, DELB(4))
C
...
C
      IF (ABS_FUNC(FL-A2B)/A2B .LE. LIMIT) GO TO 170
...
C
      ...
      ISV = 4
      CALL GMMAT_FUNC (TA,3,3,0, SMALVN(1),3,1,0, SMALVN(4))
C
...
C
  180 VECK(1) = VECI(2)*SMALVN(ISV+2) - VECI(3)*SMALVN(ISV+1)
      ...
      FLL     = SQRT_FUNC(VECK(1)**2 + VECK(2)**2 + VECK(3)**2)
      IF (ABS_FUNC(FLL) .LT. EPSI2) GO TO 7770
      DO 190 I = 1,3
...
C
      ...
      FLL     = SQRT_FUNC (VECJ(1)**2 + VECJ(2)**2 + VECJ(3)**2)
      IF (ABS_FUNC(FLL) .LT. EPSI2) GO TO 7770
      VECJ(1) = VECJ(1)/FLL
      VECJ(2) = VECJ(2)/FLL
      VECJ(3) = VECJ(3)/FLL
...
C
      EI1  = CAST_FUNC(E)*CAST_FUNC(I1)
      EI2  = CAST_FUNC(E)*CAST_FUNC(I2)
      IF (K1.EQ.0.0 .OR. I12.NE.0.0) GO TO 210
      GAK1 = CAST_FUNC(G)*CAST_FUNC(A)*CAST_FUNC(K1)
      R1   = (TWELVE*EI1*GAK1)/(GAK1*BLCUBE + TWELVE*BL*EI1)
      GO TO 220
  210 R1   = TWELVE*EI1/BLCUBE
  220 IF (K2.EQ.0.0 .OR. I12.NE.0.0) GO TO 230
      GAK2 = CAST_FUNC(G)*CAST_FUNC(A)*CAST_FUNC(K2)
      R2   = (TWELVE*EI2*GAK2)/(GAK2*BLCUBE + TWELVE*BL*EI2)
      GO TO 240
  230 R2  = TWELVE*EI2/BLCUBE
C
  240 SK1 = QUARTER*R1*BLSQ + EI1/BL
      SK2 = QUARTER*R2*BLSQ + EI2/BL
      SK3 = QUARTER*R1*BLSQ - EI1/BL
      SK4 = QUARTER*R2*BLSQ - EI2/BL
C
      AEL = CAST_FUNC(A)*CAST_FUNC(E)/BL
      LR1 = BL*R1/TWO
      LR2 = BL*R2/TWO
      GJL = CAST_FUNC(G)*CAST_FUNC(FJ)/BL
C
C
...
C
  ...
  300 KE(I) = ZERO
      ...
      BETA = TWELVE*CAST_FUNC(E)*CAST_FUNC(I12)/BLCUBE
      LB      =   BL*BETA/TWO
      L2B3    = BLSQ*BETA/THREE
      L2B6    = BLSQ*BETA/SIX
      KE( 15) =  BETA
      KE( 17) = -LB
      KE( 21) = -BETA
...
C
      IG = 1
      IF (BASIC) GO TO 380
      CALL TRANS_FUNC (ECPT(JCSID),KE(10))
      CALL GMMAT_FUNC (KE(1), 3,3,0, KE(10), 3,3,0, KE(19))
      IG = 19
C
...
C
      CALL GMMAT_FUNC (KE(IG), 3,3,0, KE(10), 3,3,0, KE(28))
C
C
...
C
      ...
      IFORE = ((I-1)/2)*36 + 37
      CALL GMMAT_FUNC (KE(IFORE), 6,6,1, KK(IKX), 6,6,0, KE(109))
      CALL GMMAT_FUNC (KE(109),  6,6,0, KE(IAFT), 6,6,0, KEP(IK))
      IAFT = 73
      IF (I .EQ. 3) IAFT = 37
  400 CONTINUE
C
...
C
  600 CONST = (FL*(CAST_FUNC(RHO)*CAST_FUNC(A) +  CAST_FUNC(NSM)))/
     1        LITERAL_420
      IF (IMASS.EQ.0 .OR. CONST.EQ.ZERO) RETURN
...
C
  800 MATIDC = IMATID
      ...
      DO 810 I = 1,72
  810 MEP(I) = ZERO
      FM = HALF*FL*(CAST_FUNC(RHO)*CAST_FUNC(A) + CAST_FUNC(NSM))
C
...
C
  500 MATFLG  = 1
      ...
      CALL HMAT (IELID)
C
      KK(1) = CAST_FUNC(FK)*CAST_FUNC(ECPT(17))/FL
      IF (KK(1).EQ. ZERO) GO TO 520
      KK(2) =-KK(1)
      KK(3) = KK(2)
      KK(4) = KK(1)
...
C
      CALL HMAT (IELID)
      KK(1) = (CAST_FUNC(FK)*CAST_FUNC(ECPT(17)))*FL/TWO
      IF (KK(1) .EQ. ZERO) RETURN
      KK(2) = KK(1)
      DICT(2) = 2
...
!
! This template demonstrates precision conversion for complex beam element:
!
! 1. Type Declarations:
...
!
! 4. Type Conversions (critical for mixed precision):
!    CAST_FUNC(E) → DBLE(E) (double) or E (single)
!    Used extensively: CAST_FUNC(G)*CAST_FUNC(A)*CAST_FUNC(K1)
!
! 5. Numeric Literals:
!    ZERO → 0.0D0 or 0.0
!    ONE → 1.0D0 or 1.0
...
!    HALF → 0.5D0 or 0.5
!    QUARTER → 0.25D0 or 0.25
!    LITERAL_01 → 0.01D0 or 0.01
!    LITERAL_13 → 13.0D0 or 13.0
!    LITERAL_22 → 22.0D0 or 22.0
!    LITERAL_35 → 35.0D0 or 35.0
!    LITERAL_54 → 54.0D0 or 54.0
!    LITERAL_156 → 156.0D0 or 156.0
!    LITERAL_175 → 175.0D0 or 175.0
!    LITERAL_420 → 420.0D0 or 420.0
!    SMALL_TOL → 1.0D-18 or 1.0E-18
!    LARGE_TOL → 1.0D-7 or 1.0E-7
!
! 6. Complexity Features:
!    - Offset vector transformation (both ends)
!    - Pin flag processing (10 possible pins)
!    - Coordinate system transformations
!    - Consistent and conventional mass formulations
!    - Heat transfer branch
!    - Cross-product coupling (I12 term)
!
! Generated files should match original bard.f and bars.f functionality.
!
!===============================================================================
