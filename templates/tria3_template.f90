!===============================================================================
! TEMPLATE: tria3_template.f90
!
! PURPOSE:
!   Single-source template for TRIA3 (3-node triangular shell) element.
!   Generates both TRIA3D (double precision) and TRIA3S (single precision)
!   from this single source file.
!
! BUILD:
!   # Generate TRIA3D (double precision):
!   gfortran -cpp -E -DDOUBLE_PRECISION tria3_template.f90 -o tria3d.f
!
!   # Generate TRIA3S (single precision):
!   gfortran -cpp -E tria3_template.f90 -o tria3s.f
!
! TEMPLATE SYSTEM:
...
!     - SQRT_FUNC, SIN_FUNC, COS_FUNC for math functions
!     - TRANS_FUNC, GMMAT_FUNC, MPYA3_FUNC for matrix operations
!     - ABS_FUNC for absolute value
!     - CAST_FUNC(X) for type conversions
!     - ZERO, ONE, TWO, etc. for numeric literals
!     - CORE_ADJUST for open core pointer adjustment (/2 for double, nothing for single)
!
! BUGFIX INCLUDED:
!   G-matrix initialization (lines 242-246 in originals) uses proper nested
!   loop structure to zero all 81 elements. This fix ensures correct behavior.
!
! ORIGINAL SOURCES:
!   - tria3d.f (548 lines) - Double precision
!   - tria3s.f (549 lines) - Single precision
!   - 99%+ duplication eliminated by this template
!
...
C     Generated from tria3_template.f90
#else
...
C
     ...
     2                 SIL(3),IORDER(3),CPMASS,MID(4),TYPE,INDEX(3,3)
      ...
      PREC_TYPE        AMGG(1),AKGG(1),ALPHA(1),THETAM,CENTE(3),
     ...
     7                 (KHEAT(4),HTCP)  , (HTCAP(1),XMASS(1))
      DATA     HUNMEG, EPS / 100000000, LITERAL_1E_M7 /
...
C
      JCORED = JCORE CORE_ADJUST + 1
      LENGTH = (NCORE-JCORE-1) CORE_ADJUST
      IF (LENGTH.LT.324 .AND. (.NOT.HEAT .AND. NEEDK)) GO TO 1100
C
C     SET UP THE ELEMENT FORMULATION
C
      CALL T3SET##PREC_SUFFIX (IERR,SIL,IGPDT,ELTH,GPTH,DGPTH,EGPDT,
     1         GPNORM,EPNORM,IORDER,TEB,TUB,CENTE,AVGTHK,LX,LY,EDGLEN,
     2         ELID)
      IF (IERR .NE. 0) GO TO 1110
      CALL GMMAT_FUNC (TEB,3,3,0, TUB,3,3,1, TEU)
      AREA = LX*LY/TWO
C
...
C
      ...
      IF ( EST(16) .EQ. 0.0) TS = LITERAL_5_6
      IF (NEST(13).EQ.0 .AND. NEST(11).GT.HUNMEG) TS = LITERAL_5_6
C
      MID(1) = NEST(11)
      MID(2) = NEST(13)
      MID(3) = NEST(15)
...
C
      MEMBRN = MID(1).GT.0
      BENDNG = MID(2).GT.0 .AND. MOMINR.GT.ZERO
      SHRFLX = MID(3).GT.0
      MBCOUP = MID(4).GT.0
      NORPTH = MID(1).EQ.MID(2) .AND. MID(1).EQ.MID(3) .AND. MID(4).EQ.0
     1         .AND. ABS_FUNC(MOMINR-ONE).LE.EPS
C
C     SET UP TRANSFORMATION MATRIX FROM MATERIAL TO ELEMENT COORD.SYSTEM
C
      CALL SHCSG##PREC_SUFFIX (*1120,NEST(9),NEST(8),NEST(8),NEST(21),
     1             NEST(20),NEST(20),NECPT,TUB,MCSID,THETAM,TUM)
C
...
C
      CALL GMMAT_FUNC (TEU,3,3,0,TUM,3,3,0,TEM)
      CALL SHGMG##PREC_SUFFIX (*1130,ELID,TEM,MID,TS,NOALFA,GI,RHO,
     1             GSUBE,TSUB0,EGNOR,ALPHA)
C
...
C
      ...
      DO 110 I = 28,36
      IF (ABS_FUNC(GI(I)) .GT. EPS) GO TO 120
  110 CONTINUE
      MBCOUP = .FALSE.
C
...
C
  120 IF (.NOT.BENDNG) GO TO 130
      CALL T3GEM##PREC_SUFFIX (IERR,EGPDT,IORDER,GI(10),GI(19),LX,LY,
     1             EDGLEN,SHRFLX,AIC,JOG,JOK,K11,K22)
      IF (IERR .NE. 0) GO TO 1110
C
...
C
      DO 140 IPT = 1,3
      KPT = (IPT-1)*ND9 + 1
      CALL T3BMG##PREC_SUFFIX (IERR,SHEART,IPT,IORDER,EGPDT,DGPTH,AIC,
     1             TH,DETJAC,SHPT,BTERMS,BMAT1(KPT))
      IF (IERR .NE. 0) GO TO 1110
  140 CONTINUE
C
      DO 150 I = 1,NDOF
...
C
  160 NEEDM = MGG1.NE.0 .AND. (NSM.GT.0.0 .OR. RHO.GT.ZERO)
...
C
      CALL T3BMG##PREC_SUFFIX (IERR,SHEART,IPT,IORDER,EGPDT,DGPTH,AIC,
     1             TH,DETJAC,SHPT,BTERMS,BMATRX)
      IF (IERR .NE. 0) GO TO 1110
C
...
C
  ...
  290 CONTINUE
C
      TSM   = ONE/(TWO*TWELVE*REALI)
      ZZ(1) = (JOG/TSI)* GI(22) + TSM*JOK*K22
      ZZ(2) =-(JOG/TSI)*(GI(20) + GI(21))/TWO
      ZZ(3) = ZERO
      ZZ(4) = ZZ(2)
      ...
      ZZ(7) = ZERO
      ZZ(8) = ZERO
      ZZ(9) = (JOG/TSI)*(GI(22) + GI(19))/TWO
     1      + TSM*TWELVE*AREA/SQRT_FUNC(GI(10)*GI(14))
...
C     INTEGRATION POINT.
C
  340 CALL T3BGB##PREC_SUFFIX (9,NDOF,G,BMATRX,AKGG(JCORED))
C
C
...
C
      DO 510 I = 1,NNODE
      IPOINT = 9*(I-1) + 1
      CALL TRANS_FUNC (IGPDT(1,I),TBG)
      CALL GMMAT_FUNC (TEB,3,3,0, TBG,3,3,0, TRANS(IPOINT))
  510 CONTINUE
C
...
C
      ...
      CALL TLDR##PREC_SUFFIX (OFFSET,II,TRANS,TMPTRN)
      DO 540 JJ = 1,36,6
      J  = JJ - 1
      KK = I - 1 + J*NNODE
      DO 530 K = 1,6
...
C
      CALL MPYA3_FUNC (TOTTRN,AKGG(JCORED),NDOF,6,TRANSK)
C
...
C     PERFORM THE TRIPLE MULTIPLY.
C
      CALL MPYA3_FUNC (TOTTRN,AMGG(JCORED),NDOF,3,TRANSK)
      GO TO 720
C
...
C
  800 CONTINUE
      INFLAG = 2
      SINMAT = SIN_FUNC(THETAM)
      COSMAT = COS_FUNC(THETAM)
      MATID  = NEST(11)
C
      CALL HMAT (ELID)
C
...
C
      ...
      IP = IP + 2
  920 CONTINUE
      CALL GMMAT_FUNC (BTERMS,2,NNODE,-1, HTFLX,NNODE,2,1, HTCON)
C
...
!
! This template demonstrates comprehensive precision conversion:
!
! 1. Type Declarations:
!    PREC_TYPE → DOUBLE PRECISION or REAL
!
! 2. Math Functions:
!    SQRT_FUNC → DSQRT or SQRT
!    SIN_FUNC, COS_FUNC → DSIN/DCOS or SIN/COS
!    ABS_FUNC → DABS or ABS
!
! 3. Matrix Operations:
!    TRANS_FUNC → TRANSD or TRANSS
!    GMMAT_FUNC → GMMATD or GMMATS
!    MPYA3_FUNC → MPYA3D or MPYA3S
!
! 4. Element-Specific Routines (append suffix):
!    T3SET##PREC_SUFFIX → T3SETD or T3SETS
!    T3BMG##PREC_SUFFIX → T3BMGD or T3BMGS
!    T3GEM##PREC_SUFFIX → T3GEMD or T3GEMS
!    T3BGB##PREC_SUFFIX → T3BGBD or T3BGBS
!    SHCSG##PREC_SUFFIX → SHCSGD or SHCSGS
!    SHGMG##PREC_SUFFIX → SHGMGD or SHGMGS
!    TLDR##PREC_SUFFIX → TLDRD or TLDRS
!
! 5. Numeric Literals:
!    ZERO → 0.0D0 or 0.0
!    ONE → 1.0D0 or 1.0
!    TWO → 2.0D0 or 2.0
!    SIX → 6.0D0 or 6.0
!    TWELVE → 12.0D0 or 12.0
!    LITERAL_1E_M7 → 1.0D-7 or 1.0E-7
!    LITERAL_5_6 → 0.833333333D0 or 0.833333333
!
! 6. Open Core Adjustment:
!    JCORE CORE_ADJUST + 1 → JCORE/2 + 1 (double) or JCORE + 1 (single)
!    (NCORE-JCORE-1) CORE_ADJUST → /2 (double) or nothing (single)
!
! 7. Bugfix Included:
!    Lines 242-246: Proper nested loop G-matrix initialization
!    DO 240 IG = 1,9
!      DO 230 JG = 1,9
!        G(IG,JG) = ZERO
!
! Generated files should be functionally identical to original tria3d.f
! and tria3s.f (with bugfix applied).
!
!===============================================================================
