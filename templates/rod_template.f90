...
#else
C     SINGLE PRECISION VERSION - Generated from rod_template.f90
#endif
C
...
C
      ...
      TE = CAST_FUNC(G*JFACT)/EL
C
C     PROCESS STIFFNESS HERE
C
      IF (KMBGG(1) .EQ. 0) GO TO 220
      IF (KE.EQ.ZERO .AND. TE.EQ.ZERO) GO TO 220
C
C     GENERATE   HA  =  (E*TA)/EL   AND  HB = (E*TB)/EL
C
      IF (IEST(9) .EQ. 0) GO TO 30
      CALL TRANS_FUNC (BGPDT(1,1),TA)
      CALL GMMAT_FUNC (EVECT,1,3,0, TA,3,3,0, HA)
      DO 20 I = 1,3
   20 HA(I) = HA(I)/EL
      GO TO 50
   30 DO 40 I = 1,3
   ...
   50 IF (IEST(13) .EQ. 0) GO TO 70
      CALL TRANS_FUNC (BGPDT(1,2),TB)
      CALL GMMAT_FUNC (EVECT,1,3,0, TB,3,3,0, HB)
      DO 60 I = 1,3
   60 HB(I) = HB(I)/EL
      GO TO 90
   70 DO 80 I = 1,3
...
C
   90 ICODE = 0
      NDOF  = 0
      IF (TE  .NE. ZERO) GO TO 100
      ICODE = 7
      NDOF  = 6
      GO TO 120
  100 IF (KE  .NE. ZERO) GO TO 110
      ICODE = 56
      NDOF  = 6
      GO TO 120
  110 ICODE = 63
...
!
! 1. Type Declarations:
!    PREC_TYPE → DOUBLE PRECISION (if -DDOUBLE_PRECISION)
!              → REAL (otherwise)
!
! 2. Math Functions:
!    SQRT_FUNC → DSQRT (double) or SQRT (single)
!
! 3. Matrix Operations:
!    TRANS_FUNC → TRANSD (double) or TRANSS (single)
!    GMMAT_FUNC → GMMATD (double) or GMMATS (single)
!
! 4. Type Conversions:
!    CAST_FUNC(E*AFACT) → DBLE(E*AFACT) (double)
!                       → E*AFACT (single, no conversion)
!
! 5. Numeric Literals:
!    ZERO → 0.0D0 (double) or 0.0 (single)
!    TWO  → 2.0D0 (double) or 2.0 (single)
!
