!===============================================================================
! TEMPLATE: quad4_template.f90
!
! PURPOSE:
!   Single-source template for QUAD4 (4-node quadrilateral shell) element
!   Generates both double and single precision variants via preprocessor
!
! GENERATES:
!   - quad4d.f (DOUBLE PRECISION) with -DDOUBLE_PRECISION flag
!   - quad4s.f (REAL single precision) without flag
!
! ORIGINAL FILES:
!   - mis/quad4d.f (1,718 lines)
!   - mis/quad4s.f (1,671 lines)
!   - Total: 3,389 lines → ~1,715 lines template (49% reduction)
!
! ELEMENT DESCRIPTION:
!   QUAD4 - 4-node isoparametric quadrilateral shell element
!   - 4 nodes with 6 DOF per node (3 translations + 3 rotations)
!   - Supports membrane, bending, transverse shear behaviors
!   - Includes material anisotropy and heat transfer capability
!   - Uses 2x2x2 Gauss integration for stiffness
!   - Consistent or lumped mass formulation
!
! BUILD COMMANDS:
!   # Double precision:
!   gfortran -cpp -E -DDOUBLE_PRECISION -I. quad4_template.f90 -o quad4d.f
!
!   # Single precision:
!   gfortran -cpp -E -I. quad4_template.f90 -o quad4s.f
!
...
C
     ...
     3                 ECPT(4),SAVE(20)
      PREC_TYPE        AMGG(1),AKGG,DGPTH(4),BMAT1(384),XYBMAT(96),
...
CWKBD 2/94 SPR93020      PREC_TYPE        EIX,EIY,TGX,TGY
CWKBI 9/94 SPR93020
      PREC_TYPE        VKL, V12DK, VP12L, VJL
CWKBI 2/94 SPR93020
      PREC_TYPE        DNUX, DNUY
C
C     PREC_TYPE        PTINTZ(2),BMATRX(144),STRESR(240)
C
C     DATA FOR ADDING ELEMENT, USER AND MATERIAL COORDINATE SYSTEMS
C
      PREC_TYPE        AA,BB,CC,X31,Y31,X42,Y42,EXI,EXJ,UGPDM(3,4),
...
C
      PREC_TYPE        RHO,TS,TSI,REALI,RHOX,THETAM,XM,YM,U(9),A,B,
...
CWKBNB 11/93 SPR 93020
      PREC_TYPE        VD1(3), VD2(3), VKN(3), VKS(3)
     1,                V12(3), V41(3), VP12(3),VIS(3), VJS(3)
CWKBNE 11/93 SPR 93020
C
C     DATA FOR IRREGULAR 4-NODE
C
      PREC_TYPE        ZC(4),UEV,ANGLEI,EDGEL,EDGSHR,UNV,VNT(3,4),CONST,
...
C    1                 (SYS(50) ,ICSUBT  ), (SYS(75) ,ICSUB8    )
     ...
     2                 (IGPDT(1,1) ,BGPDT(1,1))
      DATA    EPS1  /  LITERAL_1E_M7 /
      DATA    CONST /  GAUSS_CONST /
...
C
      ...
      VKL = SQRT_FUNC( VKN(1)**2 + VKN(2)**2 + VKN(3)**2 )
...
C
      ...
      VP12L   = SQRT_FUNC( VP12(1)**2 + VP12(2)**2 + VP12(3)**2 )
...
C
      VJL = SQRT_FUNC( VJS(1)**2 + VJS(2)**2 + VJS(3)**2 )
...
C
      ...
      DO 40 K = 1,3
      KK = IP + K
   40 UGPDM(I,J) = UGPDM(I,J) + TUB(KK)*(CAST_FUNC(BGPDM(K,J))-GGU(K))
C
C
...
C
      ...
      AA  = SQRT_FUNC(X31*X31 + Y31*Y31)
      BB  = SQRT_FUNC(X42*X42 + Y42*Y42)
      IF (AA.EQ.ZERO .OR. BB.EQ.ZERO) GO TO 1700
C
C     NORMALIZE XIJ'S
C
      X31 = X31/AA
...
C
      CALL Q4NRM_FUNC (BGPDT,GPNORM,IORDER,IFLAG)
...
C
      ...
      CALL GMMAT_FUNC (TEU,3,3,0,TUB ,3,3,0,TEB  )
      CALL GMMAT_FUNC (TUB,3,3,1,CENT,3,1,0,CENTE)
      IDENTT = 0
      IF (TEB(1).EQ.ONE .AND. TEB(5).EQ.ONE .AND. TEB(9).EQ.ONE .AND.
     1    TEB(2).EQ.ZERO .AND. TEB(3).EQ.ZERO .AND. TEB(4).EQ.ZERO .AND.
     2    TEB(6).EQ.ZERO .AND. TEB(7).EQ.ZERO .AND. TEB(8).EQ.ZERO
     3    ) IDENTT = 1
      IP = -3
      DO 170 II = 2,4
      IP = IP + 3
      ...
      KK = IP + K
      K1 = K  + 1
      CC = CAST_FUNC(BGPDT(K1,J)) - GGU(K)-CENTE(K)
...
C    1        .AND. DABS(MOMINR-ONE).LE.EPS1
C
      NORPTH = .FALSE.
C
...
C     IF (.NOT.BENDNG) GO TO 290
  ...
  210 CONTINUE
C
      A = HALF*ABS_FUNC(XA(2)+XA(3)-XA(1)-XA(4))
      B = HALF*ABS_FUNC(YB(4)+YB(3)-YB(1)-YB(2))
...
C
      ...
      ASPCTY = B/A
      CSUBB4 = LITERAL_1_6
      CSUBT  = LITERAL_71*ASPECT*(LITERAL_1_6/CSUBB4)*
     1         (ONE+LITERAL_415*ASPECT*THLEN**2)
...
C
  ...
  250 SINEAY = SINEAY+CC
      ...
      GO TO 240
  260 CC = LITERAL_28
      SINEAX = CC*SINEAX + ONE
      SINEAY = CC*SINEAY + ONE
...
C
      ...
      UEV(2,I) = YB(J) - YB(I)
      UEV(3,I) = ZC(J) - ZC(I)
      UNV(1,I) = (VNT(1,J) + VNT(1,I))*HALF
      UNV(2,I) = (VNT(2,J) + VNT(2,I))*HALF
      UNV(3,I) = (VNT(3,J) + VNT(3,I))*HALF
      CC = UEV(1,I)**2 + UEV(2,I)**2 + UEV(3,I)**2
      IF (CC .EQ. ZERO) GO TO 1700
      IF (CC .GE. EPS1) CC = SQRT_FUNC(CC)
      EDGEL(I) = CC
      UEV(1,I) = UEV(1,I)/CC
      UEV(2,I) = UEV(2,I)/CC
      UEV(3,I) = UEV(3,I)/CC
...
C
      ...
      IF (ABS_FUNC(ANGLEI(I)) .LT .EPS1) ANGLEI(I) = ZERO
  280 CONTINUE
C
...
C
      ...
      ETA = PTINT(IETA)
C
      CALL Q4SHP_FUNC (XI,ETA,SHP,DSHP)
C
...
C
     ...
     1                (ONE-ANGLEI(IJ)*ANGLEI(IJ))
     2              + (UEV(IS,IJ)+ANGLEI(IK)*UEV(IS,IK))*BB/
     3                (ONE-ANGLEI(IK)*ANGLEI(IK))
  300 CONTINUE
  310 CONTINUE
C
...
C
      CALL GMMAT_FUNC (PSITRN,3,3,0,JACOB,3,3,1,PHI)
C
C     CALL Q4BMGD TO GET B MATRIX
C     SET THE ROW FLAG TO 1. IT SIGNALS SAVING THE LAST 2 ROWS.
C
      ROWFLG = 1
      CALL Q4BMG_FUNC (DSHP,DGPTH,EGPDT,EPNORM,PHI,BMAT1(KPT))
  340 KPT = KPT + ND2
  350 CONTINUE
C
...
C
      ROWFLG = 2
      CALL Q4BMG_FUNC (DSHP,DGPTH,EGPDT,EPNORM,PHI,XYBMAT(KPT))
  390 KPT = KPT + KPNT
C
...
C
  ...
  430 TEM(I) = TEB(I)
      GO TO 450
  440 NECPT(1) = MCSID
      CALL TRANS_FUNC (ECPT,TBM)
C
C     MULTIPLY TEB AND TBM MATRICES
C
      CALL GMMAT_FUNC (TEB,3,3,0,TBM,3,3,0,TEM)
C
...
C
  450 CONTINUE
      XM = TEM(1)
      YM = TEM(4)
      IF (ABS_FUNC(XM).GT.EPS1 .OR. ABS_FUNC(YM).GT.EPS1) GO TO 460
      NEST(2) = MCSID
      J = 231
      GO TO 1720
  460 THETAM = ATAN2_FUNC(YM,XM)
      GO TO 480
C
C     CALCULATE TEM-MATRIX USING THETAM
C
  470 THETAM = CAST_FUNC(EST(10))*DEGRAD
C     IF (THETAM .EQ. ZERO) GO TO 490
      IF (THETAM .EQ. ZERO) GO TO 490
  480 CALL ANGTR_FUNC (THETAM,1,TUM)
...
C
  490 IF (NEST(24) .EQ. 0) GO TO 500
      ...
      GO TO 420
C
  500 THETAM = CAST_FUNC(EST(23))*DEGRAD
      GO TO 480
C
  510 CONTINUE
      IF (HEAT) GO TO 1810
...
C
      ...
      LPOINT = (M-1)*9 + 1
C
      CALL Q4GMG_FUNC (M,COEFF,GI(LPOINT))
C
...
C      GNORY = MATOUT(5)
C      IF (GNORX .EQ. ZERO) GNORX = MATOUT(4)
C      IF (GNORY .EQ. ZERO) GNORY = MATOUT(4)
C  760 CONTINUE
C
...
C
  770 U(1) = TEM(5)*TEM(9) + TEM(6)*TEM(8)
      ...
      L    = 2
C
  780 CALL GMMAT_FUNC (U(1),L,L,1,GI(LPOINT),L,L,0,GT(1))
      CALL GMMAT_FUNC (GT(1),L,L,0,U(1),L,L,0,GI(LPOINT))
CWKBNB 11/93 SPR93020
      IF (M .GT. 0) GO TO 670
...
CWKBNE 2/94 SPR93020
  ...
  750 GNORX = MATOUT(6)
      GNORY = MATOUT(5)
      IF ( GNORX .EQ. ZERO ) GNORX = MATOUT(4)
      IF ( GNORY .EQ. ZERO ) GNORY = MATOUT(4)
  760 CONTINUE
      GO TO 610
C
C     END OF M-LOOP
C
  790 CONTINUE
      ...
      IF (GI(19).NE.ZERO .OR. GI(20).NE.ZERO .OR. GI(21).NE.ZERO .OR.
     1    GI(22).NE.ZERO) GO TO 800
      IGOBK = 1
      M = 2
      MID(3) = MID(2)
      GO TO 610
  800 CONTINUE
C
      NOCSUB = ENORX.EQ.ZERO .OR. ENORY.EQ.ZERO .OR.
     1         GNORX.EQ.ZERO .OR. GNORY.EQ.ZERO .OR.
     2        MOMINR.EQ.ZERO
C
      MATTYP = IFIX(MATSET+.05)
C
C     IF MGG1 IS NON-ZERO AND RHO IS GREATER THAN ZERO,
...
C
  ...
  910 DSHP(I+4) = DSHPTP(KK+4)
      CALL GMMAT_FUNC (SHP,1,NNODE,0,DGPTH,1,NNODE,1,THK)
      REALI = MOMINR*THK*THK*THK/TWELVE
C     REALI =        THK*THK*THK/TWELVE
      TSI   = TS*THK
C
...
C
      ...
      IF (RHO .EQ. ZERO) GO TO 1020
      IF (RHO .GT. ZERO) GO TO 920
...
CWKBR 11/93 SPR 93015 IF (AREA2 .LT. EPS1) GO TO 1700
      IF ( AREA2 .LE. ZERO ) GO TO 1700
C
      AREA2 = SQRT_FUNC(AREA2)
      AREA  = AREA + AREA2
      VOLI  = AREA2*THK
      VOL   = VOL + VOLI
C
...
C
 1000 CALL GMMAT_FUNC (SHP,1,NNODE,1,SHP,1,NNODE,0,XMTMP)
C
...
C      NUNORX = MOMINR*ENORX/(TWO*GNORX) - ONE
C      NUNORY = MOMINR*ENORY/(TWO*GNORY) - ONE
CWKBNB 2/94 SPR93020
      NUNORX = MOMINR*ENORX/(TWO*GNORX) - ONE
      NUNORY = MOMINR*ENORY/(TWO*GNORY) - ONE
CWKBNE 2/94 SPR93020
C
...
C      TGX = TWO*GNORX
C      TGY = TWO*GNORY
C      NUNORX = EIX/TGX - ONE
C      NUNORY = EIY/TGY - ONE
C      IF (EIX .GT. TGX) NUNORX= ONE - TGX/EIX
C      IF (EIY .GT. TGY) NUNORY= ONE - TGY/EIY
...
C
      W1 = ONE + 4400.0*THLEN*THLEN*THLEN*THLEN
...
C
C     W1 = ONE + 2.5*THLEN + 1.0D04*THLEN**5
C     IF (CC .LT. 0.2) GO TO 1030
C     DSUB4 = 18.0*W1
C     GO TO 1040
C1030 DSUB4 = (179.85*CC-17.97)*W1
 1040 IF (DSUB4 .LT. LITERAL_01) DSUB4 = LITERAL_01
      IF (DSUB4 .GT. 2.0D3) DSUB4 = 2000.0
      DSUB  = DSUB4
      COEFT = CONST
      AX    = A
      IF (ETA .LT .ZERO) AX = A + COEFT*(XA(2)-XA(1)-A)
      IF (ETA .GT. ZERO) AX = A + COEFT*(XA(3)-XA(4)-A)
      PSIINX = 20.0*DSUB*REALI*SINEAX*(ONE+ASPECT*ASPECT)/
     1         (TSI*(ONE-NUNORX)*AX*AX)
      DSUB  = DSUB4
      COEFT = CONST
      BY    = B
      IF (XI .LT. ZERO) BY = B + COEFT*(YB(4)-YB(1)-B)
      IF (XI .GT. ZERO) BY = B + COEFT*(YB(3)-YB(2)-B)
      PSIINY = 20.0*DSUB*REALI*SINEAY*(ONE+ASPECT*ASPECT)/
     1         (TSI*(ONE-NUNORY)*BY*BY)
      IF (.NOT.SHRFLX) GO TO 1050
      TSMFX = PSIINX/(ONE+PSIINX)
      TSMFY = PSIINY/(ONE+PSIINY)
      GO TO 1060
 1050 TSMFX = PSIINX
      TSMFY = PSIINY
C
 1060 CONTINUE
      IF (TSMFX .LE. ZERO) TSMFX = EPS1
      IF (TSMFY .LE. ZERO) TSMFY = EPS1
C
...
C
      CSUBX  = 20.0*REALI/(TSI*(ONE-NUNORX)*A*A)
      CSUBY  = 20.0*REALI/(TSI*(ONE-NUNORY)*B*B)
      SFCTR1 = CSUBB4*CSUBX
      SFCTR2 = CSUBTX*CSUBX
      IF (.NOT.SHRFLX) GO TO 1220
      SFCTR1 = SFCTR1/(ONE+SFCTR1)
      SFCTR2 = SFCTR2/(ONE+SFCTR2)
 1220 CONTINUE
...
C
      ...
      GFOUR( 7, 7) = QUARTER*SFCTY1*TS*GI(19)
      GFOUR( 8, 8) = QUARTER*SFCTY1*TS*GI(19)
      GFOUR( 8, 7) = QUARTER*SFCTY2*TS*GI(19)
      GFOUR( 7, 8) = GFOUR(8,7)
      GFOUR( 9, 9) = QUARTER*SFCTX1*TS*GI(22)
      GFOUR(10,10) = QUARTER*SFCTX1*TS*GI(22)
      GFOUR(10, 9) = QUARTER*SFCTX2*TS*GI(22)
      GFOUR( 9,10) = GFOUR(10,9)
      GFOUR( 7, 9) = SQRT_FUNC(TSMFX*TSMFY)*TS*GI(20)
      GFOUR( 9, 7) = GFOUR(7,9)
      GO TO 1250
C
 1240 DFOUR(4,4) = QUARTER*SFCTY1*TS*GI(19)
      DFOUR(5,5) = QUARTER*SFCTY1*TS*GI(19)
      DFOUR(5,4) = QUARTER*SFCTY2*TS*GI(19)
      DFOUR(4,5) = DFOUR(5,4)
      DFOUR(6,6) = QUARTER*SFCTX1*TS*GI(22)
      DFOUR(7,7) = QUARTER*SFCTX1*TS*GI(22)
      DFOUR(7,6) = QUARTER*SFCTX2*TS*GI(22)
      DFOUR(6,7) = DFOUR(7,6)
      DFOUR(4,6) = SQRT_FUNC(TSMFX*TSMFY)*TS*GI(20)
      DFOUR(6,4) = DFOUR(4,6)
 1250 CONTINUE
C
...
C
      ...
      JG1 = JG4 - 27
      LG  = JG  + 3
      GFOUR(IG,LG) = -GI(JG4)*ZTA*SIX+GI(JG1)
 1280 GFOUR(KG,JG) = -GI(JG4)*ZTA*SIX+GI(JG1)
 1290 CONTINUE
C
C     COMPUTE THE JACOBIAN AT THIS GAUSS POINT,
C     ITS INVERSE AND ITS DETERMINANT.
...
C
      ROWFLG = 3
      CALL Q4BMG_FUNC (DSHP,DGPTH,EGPDT,EPNORM,PHI,BFOUR(1))
C
...
C
C
      CALL TRPLM_FUNC (GFOUR,DFOUR,BFOUR,BMAT1,XYBMAT,MATTYP,JCORED,
     1                 DETJ)
...
C
      ...
      I = IM1 + JJ
      J = (JJ-1)*NDOF + II + IJ
      TEMP = (AKGG(I) + AKGG(J))*HALF
      IF (ABS_FUNC(TEMP) .LT. 1.0D-17) TEMP = ZERO
      AKGG(I) = TEMP
      AKGG(J) = TEMP
 1320 CONTINUE
C
...
C
 1330 CONTINUE
      ...
      IF (RHO.EQ.ZERO .AND. NSM.EQ.0.0) GO TO 1410
C     IF (CPMASS .GT. 0) GO TO 1410
      IF (NSM .EQ.  0.0) GO TO 1410
      IF (VOL.EQ.ZERO .OR. RHOX.EQ.ZERO) WRITE (NOUT,2060) SFM,ELID,
...
C     AND MATRICES.
C
      CALL TRANS_FUNC (GPTH,TBG)
      CALL GMMAT_FUNC (TEB,3,3,0,TBG,3,3,0,TRANS(IPOINT))
      GO TO 1450
C
 1420 IF (IDENTT.NE.1 .OR. OFFSET.NE.ZERO) GO TO 1430
      NOTRAN(I) = 1
      GO TO 1450
C
 1430 DO 1440 J = 1,9
...
C
      CALL TLDR_FUNC (OFFSET,I,TRANS,TRANS1)
C
C     TRANSFORM THE NODAL COLUMN PARTITION.
C
      CALL GMMAT_FUNC (COLSTF,NDOF,6,0,TRANS1,6,6,0,COLTMP)
      DO 1510 II = 1,NPART
...
C     LOAD THE 6X6 TRANSFORMATION
C
      CALL TLDR_FUNC (OFFSET,M,TRANS,TRANS1)
C
C     TRANSFORM THE 6 ROWS FOR THIS SUBPARTITION
C
      CALL GMMAT_FUNC (TRANS1,6,6,1,COLSTF(MPOINT),6,6,0,COLTMP)
      IIPNT = MPOINT - 1
      DO 1520 II = 1,36
...
C
      DO 1640 M = 1,NNODE
      MPOINT = 9*(M-1) + JCORED
      CALL GMMAT_FUNC (AMGG(MPOINT),3,3,0,TRANS(9*I-8),3,3,0,TMPMAS)
      IICORE = MPOINT - 1
      DO 1630 K = 1,9
...
C
      CALL GMMAT_FUNC (TRANS(9*M-8),3,3,1,AMGG(MPOINT),3,3,0,TMPMAS)
      IICORE = MPOINT - 1
      DO 1650 K = 1,9
...
C
      ...
      MATID  = NEST(13)
      CALL HMAT (ELID)
      GI(1) = CAST_FUNC(KHEAT(1))
      GI(2) = CAST_FUNC(KHEAT(2))
      GI(3) = GI(2)
      GI(4) = CAST_FUNC(KHEAT(3))
...
C
 ...
 1810 CONTINUE
      TEM(3) = TEM(4)
      TEM(4) = TEM(5)
      CALL GMMAT_FUNC (TEM,2,2,0,GI,2,2,0,GT)
      CALL GMMAT_FUNC (GT,2,2,0,TEM,2,2,1,GI)
 1820 CONTINUE
      DO 1830 I = 1,16
      HTCON(I) = ZERO
      HTCAP(I) = ZERO
...
C
      ...
      ZETA = PTINT(IZTA)
C
      CALL TERMS_FUNC (NNODE,DGPTH,EPNORM,EGPDT,HORDER,HSIL,BTERMS)
      DVOL = DETERM
C
      DO 1850 I = 1,4
