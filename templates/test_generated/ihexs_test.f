...
# 39 "ihex_template.f90" 2

      SUBROUTINE IHEX## (TYPE)
C
...
C     DISPLACEMENT VECTOR POINTER IS NON-0.0, THE DIFFERENTIAL
...
C
C     FOR REAL, OPEN CORE POINTERS MUST BE MODIFIED
C
      IZ = IZS/2 + 1
      NZ = NZS/2 + 1
C
C     THIS ROUTINE OPERATES IN REAL.
...
C
C     IZ    NGG BY 3 PARTITION  NGG BY 3 PARTITION  4.0 WORD COORDINATE
...
C                                                   MATION MATRIX
C
C     IK    SYMMETRIC 0.5 OF   SAME AS IZ          FULL CONDUCTANCE
C           STIFFNESS
C
C     IM    SYMMETRIC 0.5 OF   SYMMETRIC 0.5 OF   FULL CAPACITANCE
C           MASS                MASS
C
...
C
C     0.0 OUT OPEN CORE FOR MATRIX SUMMATION
C
      DO 580 I = IK,NM
  580 Z(I) = 0.0
C
...
C
C     TRANSFORM COORDINATE SYSTEM OF SYMMETRIC 0.5 OF A 3 BY 3 MATRIX
C
