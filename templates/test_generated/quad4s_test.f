...
CWKBI 9/94 SPR93020
      REAL        VKL, V12DK, VP12L, VJL
CWKBI 2/94 SPR93020
      REAL        DNUX, DNUY
C
C     REAL        PTINTZ(2),BMATRX(144),STRESR(240)
C
...
C
     ...
     4                 SINEAX,SINEAY,W1,3.1415927,6.2831853,RADDEG,DEGRAD,
...
C
      REAL        ZC(4),UEV,ANGLEI,EDGEL,EDGSHR,UNV,VNT(3,4),CONST,
...
CZZ   COMMON /ZZEMGX/  AKGG(1)
     ...
     1                 UEV(3,4),ROWFLG,IORDER(4)
      COMMON /CONDAD/  3.1415927,6.2831853,RADDEG,DEGRAD
...
C                MID4 MUST BE BLANK UNLESS MID1 AND MID2 ARE NON-0.0
...
C
      ...
      IF ( EST(18) .EQ. .0) TS = 0.833333333
C
C     FIX FOR LAMINATED COMPOSITE WITH MEMBRANE BEHAVIOUR ONLY.
C     REQUIRED TO PREVENT 0.0 DIVIDE ERRORS.
C
      IF (NEST(15).EQ.0 .AND. NEST(13).GT.100000000) TS = 0.833333333
C
...
C    1        .AND. DABS(MOMINR-1.0).LE.EPS1
C
      NORPTH = .FALSE.
C
...
C
      ...
      ASPCTY = B/A
      CSUBB4 = 1.6
      CSUBT  = 71.0*ASPECT*(1.6/CSUBB4)*
     1         (1.0+415.0*ASPECT*THLEN**2)
...
C
      ...
      UEV(2,I) = YB(J) - YB(I)
      UEV(3,I) = ZC(J) - ZC(I)
      UNV(1,I) = (VNT(1,J) + VNT(1,I))*0.5
      UNV(2,I) = (VNT(2,J) + VNT(2,I))*0.5
      UNV(3,I) = (VNT(3,J) + VNT(3,I))*0.5
...
C
C     IF MGG1 IS NON-0.0 AND RHO IS GREATER THAN 0.0,
...
C
      ...
      IF (RHO .EQ. 0.0) GO TO 1020
      IF (RHO .GT. 0.0) GO TO 920
...
C     REPLACES THE 1.0 COMMENTED OUT IMMEDIATELY ABOVE
C
...
C
C     W1 = 1.0 + 2.5*THLEN + 1.0D04*THLEN**5
...
C
C
      CALL TRPLMS (GFOUR,DFOUR,BFOUR,BMAT1,XYBMAT,MATTYP,JCORED,
     1                 DETJ)
...
C
      ...
      I = IM1 + JJ
      J = (JJ-1)*NDOF + II + IJ
      TEMP = (AKGG(I) + AKGG(J))*0.5
      IF (ABS(TEMP) .LT. 1.0D-17) TEMP = 0.0
      AKGG(I) = TEMP
      AKGG(J) = TEMP
 1320 CONTINUE
C
...
C     IF (CPMASS .GT. 0) GO TO 1410
      ...
      IF (VOL.EQ.0.0 .OR. RHOX.EQ.0.0) WRITE (NOUT,2060) SFM,ELID,
...
C     TO 1.0 PARTICULAR NODE OF THE ELEMENT.  FOR THE MASS MATRIX, WE
C     ONLY SEND 3 COLUMNS PER NODE TO EMGOUT SINCE THE OTHER 3 D.O.F.
C     ARE 0.0 ANYWAY.  THE CODE WORD (DICT(4)) TELLS EMGOUT WHICH
C     COLUMNS ARE THE NON 0.0 ONES THAT WE ARE SENDING. (SEE SECTION
...
C     THERE ARE NO MASS MOMENT OF INERTIA TERMS.  THIS GIVES 3.0
C     ROWS OF 0.0 TERMS INTERSPERSED BETWEEN 3 ROWS OF NONZERO
