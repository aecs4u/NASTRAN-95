...
C
      ...
      IF (KE.EQ.0.0 .AND. TE.EQ.0.0) GO TO 220
C
...
C
   90 ICODE = 0
      NDOF  = 0
      IF (TE  .NE. 0.0) GO TO 100
      ICODE = 7
      NDOF  = 6
      GO TO 120
  100 IF (KE  .NE. 0.0) GO TO 110
      ICODE = 56
      NDOF  = 6
      GO TO 120
  110 ICODE = 63
...
!
! 1. Type Declarations:
!    REAL → DOUBLE PRECISION (if -DDOUBLE_PRECISION)
!              → REAL (otherwise)
!
! 2. Math Functions:
!    SQRT → DSQRT (double) or SQRT (single)
!
! 3. Matrix Operations:
!    TRANSS → TRANSD (double) or TRANSS (single)
!    GMMATS → GMMATD (double) or GMMATS (single)
!
! 4. Type Conversions:
!    E*AFACT → DBLE(E*AFACT) (double)
!                       → E*AFACT (single, no conversion)
!
! 5. Numeric Literals:
!    0.0 → 0.0D0 (double) or 0.0 (single)
!    2.0  → 2.0D0 (double) or 2.0 (single)
!
