...
C
     ...
     4                 SINEAX,SINEAY,W1,3.141592653589793D0,6.283185307179586D0,RADDEG,DEGRAD,
...
CZZ   COMMON /ZZEMGX/  AKGG(1)
     ...
     1                 UEV(3,4),ROWFLG,IORDER(4)
      COMMON /CONDAD/  3.141592653589793D0,6.283185307179586D0,RADDEG,DEGRAD
...
C
      ...
      IF (AA.EQ.0.0D0 .OR. BB.EQ.0.0D0) GO TO 1700
C
C     NORMALIZE XIJ'S
C
      X31 = X31/AA
...
C                MID4 MUST BE BLANK UNLESS MID1 AND MID2 ARE NON-0.0D0
...
C
      ...
      IF ( EST(18) .EQ. .0) TS = 0.833333333D0
C
C     FIX FOR LAMINATED COMPOSITE WITH MEMBRANE BEHAVIOUR ONLY.
C     REQUIRED TO PREVENT 0.0D0 DIVIDE ERRORS.
C
      IF (NEST(15).EQ.0 .AND. NEST(13).GT.100000000) TS = 0.833333333D0
C
...
C
      ...
      IDENTT = 0
      IF (TEB(1).EQ.1.0D0 .AND. TEB(5).EQ.1.0D0 .AND. TEB(9).EQ.1.0D0 .AND.
     1    TEB(2).EQ.0.0D0 .AND. TEB(3).EQ.0.0D0 .AND. TEB(4).EQ.0.0D0 .AND.
     2    TEB(6).EQ.0.0D0 .AND. TEB(7).EQ.0.0D0 .AND. TEB(8).EQ.0.0D0
     3    ) IDENTT = 1
      IP = -3
      DO 170 II = 2,4
      IP = IP + 3
...
C
      ...
      ASPCTY = B/A
      CSUBB4 = 1.6D0
      CSUBT  = 71.0D0*ASPECT*(1.6D0/CSUBB4)*
     1         (1.0D0+415.0D0*ASPECT*THLEN**2)
...
C
      ...
      UEV(2,I) = YB(J) - YB(I)
      UEV(3,I) = ZC(J) - ZC(I)
      UNV(1,I) = (VNT(1,J) + VNT(1,I))*0.5D0
      UNV(2,I) = (VNT(2,J) + VNT(2,I))*0.5D0
      UNV(3,I) = (VNT(3,J) + VNT(3,I))*0.5D0
      CC = UEV(1,I)**2 + UEV(2,I)**2 + UEV(3,I)**2
      IF (CC .EQ. 0.0D0) GO TO 1700
...
C     THE 2.0D0 INTEGRATION POINTS.
C
      ROWFLG = 2
...
C
  790 CONTINUE
      ...
      IF (GI(19).NE.0.0D0 .OR. GI(20).NE.0.0D0 .OR. GI(21).NE.0.0D0 .OR.
     1    GI(22).NE.0.0D0) GO TO 800
      IGOBK = 1
      M = 2
      MID(3) = MID(2)
      GO TO 610
...
C
C     IF MGG1 IS NON-0.0D0 AND RHO IS GREATER THAN 0.0D0,
...
C
      ...
      IF (RHO .EQ. 0.0D0) GO TO 1020
      IF (RHO .GT. 0.0D0) GO TO 920
...
C     REPLACES THE 1.0D0 COMMENTED OUT IMMEDIATELY ABOVE
C
      W1 = 1.0D0 + 4400.0*THLEN*THLEN*THLEN*THLEN
...
C
C     W1 = 1.0D0 + 2.5*THLEN + 1.0D04*THLEN**5
C     IF (CC .LT. 0.2) GO TO 1030
C     DSUB4 = 18.0*W1
C     GO TO 1040
C1030 DSUB4 = (179.85*CC-17.97)*W1
 1040 IF (DSUB4 .LT. 0.01D0) DSUB4 = 0.01D0
      ...
      PSIINX = 20.0*DSUB*REALI*SINEAX*(1.0D0+ASPECT*ASPECT)/
     1         (TSI*(1.0D0-NUNORX)*AX*AX)
      ...
      PSIINY = 20.0*DSUB*REALI*SINEAY*(1.0D0+ASPECT*ASPECT)/
...
C
      CSUBX  = 20.0*REALI/(TSI*(1.0D0-NUNORX)*A*A)
      CSUBY  = 20.0*REALI/(TSI*(1.0D0-NUNORY)*B*B)
      SFCTR1 = CSUBB4*CSUBX
      SFCTR2 = CSUBTX*CSUBX
...
C
C
      CALL TRPLMD (GFOUR,DFOUR,BFOUR,BMAT1,XYBMAT,MATTYP,JCORED,
     1                 DETJ)
...
C
 1330 CONTINUE
      ...
      IF (RHO.EQ.0.0D0 .AND. NSM.EQ.0.0) GO TO 1410
C     IF (CPMASS .GT. 0) GO TO 1410
      IF (NSM .EQ.  0.0) GO TO 1410
      IF (VOL.EQ.0.0D0 .OR. RHOX.EQ.0.0D0) WRITE (NOUT,2060) SFM,ELID,
...
C     TO 1.0D0 PARTICULAR NODE OF THE ELEMENT.  FOR THE MASS MATRIX, WE
C     ONLY SEND 3 COLUMNS PER NODE TO EMGOUT SINCE THE OTHER 3 D.O.F.
C     ARE 0.0D0 ANYWAY.  THE CODE WORD (DICT(4)) TELLS EMGOUT WHICH
C     COLUMNS ARE THE NON 0.0D0 ONES THAT WE ARE SENDING. (SEE SECTION
...
C     THERE ARE NO MASS MOMENT OF INERTIA TERMS.  THIS GIVES 3.0D0
C     ROWS OF 0.0D0 TERMS INTERSPERSED BETWEEN 3 ROWS OF NONZERO
