...
C
      CALL T3SET##D (IERR,SIL,IGPDT,ELTH,GPTH,DGPTH,EGPDT,
...
C
      CALL SHCSG##D (*1120,NEST(9),NEST(8),NEST(8),NEST(21),
...
C
  120 IF (.NOT.BENDNG) GO TO 130
      CALL T3GEM##D (IERR,EGPDT,IORDER,GI(10),GI(19),LX,LY,
...
C
      DO 140 IPT = 1,3
      KPT = (IPT-1)*ND9 + 1
      CALL T3BMG##D (IERR,SHEART,IPT,IORDER,EGPDT,DGPTH,AIC,
...
C     INTEGRATION POINT.
C
  340 CALL T3BGB##D (9,NDOF,G,BMATRX,AKGG(JCORED))
C
C
...
C
      ...
      ADAMP   = GSUBE
C
C     BUILD THE 18X18 TRANSFORMATION MATRIX FOR 1.0D0-SHOT MULTIPLY
C
      DO 520 I = 1,NPART
      TRANSK(I) = 0.0D0
      TOTTRN(I) = 0.0D0
...
C
      ...
      CALL TLDR##D (OFFSET,II,TRANS,TMPTRN)
      DO 540 JJ = 1,36,6
      J  = JJ - 1
      KK = I - 1 + J*NNODE
      DO 530 K = 1,6
...
C
  600 IF (.NOT.NEEDM) GO TO 730
      ...
      JEND    = JCORED + NPART - 1
C
C     0.0D0 OUT THE POSITIONS, THEN LOOP ON I AND J TO LOAD THE MASS
C     MATRIX.
C
...
C
C     BUILD THE 9X9 TRANSFORMATION MATRIX FOR 1.0D0-SHOT MULTIPLY
C
      DO 650 I = 1,NPART
      TRANSK(I) = 0.0D0
      TOTTRN(I) = 0.0D0
...
! 2. Math Functions:
!    DSQRT → DSQRT or SQRT
!    DSIN, DCOS → DSIN/DCOS or SIN/COS
!    DABS → DABS or ABS
!
! 3. Matrix Operations:
!    TRANSD → TRANSD or TRANSS
!    GMMATD → GMMATD or GMMATS
!    MPYA3D → MPYA3D or MPYA3S
!
! 4. Element-Specific Routines (append suffix):
!    T3SET##D → T3SETD or T3SETS
!    T3BMG##D → T3BMGD or T3BMGS
!    T3GEM##D → T3GEMD or T3GEMS
!    T3BGB##D → T3BGBD or T3BGBS
!    SHCSG##D → SHCSGD or SHCSGS
!    SHGMG##D → SHGMGD or SHGMGS
!    TLDR##D → TLDRD or TLDRS
!
! 5. Numeric Literals:
!    0.0D0 → 0.0D0 or 0.0
!    1.0D0 → 1.0D0 or 1.0
...
!    0.833333333D0 → 0.833333333D0 or 0.833333333
!
! 6. Open Core Adjustment:
!    JCORE /2 + 1 → JCORE/2 + 1 (double) or JCORE + 1 (single)
!    (NCORE-JCORE-1) /2 → /2 (double) or nothing (single)
!
! 7. Bugfix Included:
