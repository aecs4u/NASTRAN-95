...
C     DEBUG TRACING - ADDED 2026-02-09
      WRITE(6,*) 'DEBUG SEMINT: Calling NASCAR (read NASTRAN card)...'
      CALL FLUSH(6)
C
...
C
C     DEBUG TRACING
      WRITE(6,*) 'DEBUG SEMINT: NASCAR returned'
      CALL FLUSH(6)
C
...
C     DEBUG TRACING
      WRITE(6,*) 'DEBUG SEMINT: Calling GNFIAT (file tables)...'
      CALL FLUSH(6)
C
...
C
C     DEBUG TRACING
      WRITE(6,*) 'DEBUG SEMINT: GNFIAT returned'
      CALL FLUSH(6)
C
...
C
C     TEMPORARY FIX 2026-02-09: Skip timing calibration (hangs in TMTSIO)
C     These routines just calibrate performance metrics - not required for execution
      WRITE(6,*) 'DEBUG: Skipping TMTSIO/TMTSLP (timing calibration)'
      CALL FLUSH(6)
C
C      CALL CONMSG (BCD6,1,0)
C      IF (DEBUG1 .GT. 0) WRITE (6,10) SUBR(3)
C      CALL TMTSIO (*2000,DEBUG1)
C      CALL CONMSG (BCD7,1,0)
C      IF (DEBUG1 .GT. 0) WRITE (6,10) SUBR(4)
C      CALL TMTSLP
C
...
C     DEBUG TRACING
      WRITE(6,*) 'DEBUG SEMINT: Calling XCSA (exec control)...'
      CALL FLUSH(6)
C
...
C
C     DEBUG TRACING
      WRITE(6,*) 'DEBUG SEMINT: XCSA returned'
      CALL FLUSH(6)
C
